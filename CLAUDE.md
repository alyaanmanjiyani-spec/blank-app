...
| D-008 | 2026-09-21 | Sheet created by hand, then `clasp clone-script` (not `clasp create-script`) | Keeps control of where the Sheet lives in Drive, makes the bound-script relationship visible once, and isolates failures: if cloning breaks, login already worked. Costs ~2 minutes, once. |
